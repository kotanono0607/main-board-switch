// === Pleasanter サイドカーAPIクライアント (45173テーブル用) Ver1 ===
...
(function (w) {
  ...
  async function レコード取得() {
    ...
    for (var i = 0; i < 2000; i++) {
      ...
      console.log("45173 取得: offset=" + ページ.currentOf + ", pageSize=" + ページ.pageSize + ", 収集=" + 収集.length + "/" + (total || "?"));
  ...
  console.log("▶ PleasanterApi別 初期化完了（テーブルID: " + 設定.tableId + "）");
  ...
  (function 自動取得() {
    ...
    async function 取得して出力() {
      try {
        var recs = await w.PleasanterApi別.レコード取得();
        console.log("★ 別テーブル(45173) 件数:", recs.length);
        console.log("★ 先頭サンプル(最大5件):", recs.slice(0, 5));
      } catch (e) {
        console.error("別テーブル(45173) 取得失敗:", e && e.message ? e.message : e);
      }
    }
  })();
